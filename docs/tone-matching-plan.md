...
> **Status:** M7-1's atlases were built, measured and then removed from the
> repository — see "M7-1 closed" at the end of this section. M7-2 is a negative
> result.
...
and the atlas's switch and selector positions are put back after the inversion
in every arm, so the two searches differ only in their starting values. Odd
targets run the atlas arms first, so render order cannot favour one side. One
structural bias is left: targets are uniform over each control's range and the
neutral start is the centre of it, which favours the neutral start on any
control the search leaves frozen.

Holding the topology is not optional. The first runs let the inversion switch
effects on, and in half of the played-DI searches it added a rack reverb the
targets did not have — which on a played DI does not repeat itself on a reused
instance (the rack reverb note further down), so the screen froze a third to
four-fifths of the controls against its noise. Those runs pointed the same way
as the ones below; the numbers here are the reruns with the topology held.

SW50R, 12 targets, a 300-render budget, two workers, Morgan 1.1.1 through the
reused Swift server (`reproducible=False`). Three runs: the committed 1,024-point
noise-probe atlas against targets from its own noise probe, the same atlas
against targets from the played How Long passage used above (6 s from 10.66 s),
and a 128-point atlas built from that same played passage (below) against the
same played targets. Mean / median `unpaired-v1`:

| arm | noise atlas, noise targets | noise atlas, played targets | played atlas, played targets |
|---|---:|---:|---:|
| `recipe` (neutral settings) | 1.700 / 1.747 | 1.665 / 1.807 | 1.676 / 1.802 |
| `inversion` | 0.713 / 0.665 | 0.871 / 0.766 | 0.886 / 0.801 |
| `full` (neutral → inversion → search) | 0.311 / 0.326 | **0.447 / 0.453** | **0.480 / 0.429** |
| `atlas` (lookup alone) | 0.621 / 0.633 | 1.319 / 1.329 | 0.865 / 0.710 |
| `atlas-inversion` | 0.448 / 0.390 | 0.786 / 0.773 | 0.642 / 0.602 |
| `atlas-full` (atlas → inversion → search) | 0.280 / 0.266 | **0.548 / 0.581** | **0.479 / 0.437** |
| `atlas-full` closer than `full` | 8 of 12 | **2 of 12** | 6 of 12 |
...
better start gave, and building that atlas cost 128 renders before any search.
The lookup alone never beat a full search in any run. Between runs, the same
targets' `full` arm moved by 0.11 on average, so only the within-run pairings are
comparisons.

**The played-DI atlas.** Built exactly as the committed atlases, with the played
passage as `--probe-di`: it clears its own gate, 0.799 against 1.991 neutral on
24 of 24. Looked up against targets rendered from a *different* played passage —
the Hotel California DI, 6 s from 18.9 s — it beat neutral on 12 of 16, where the
committed noise atlases managed 8 (128 points) and 6 (1,024); but a few large
misses left its mean worse than neutral (1.620 against 1.483). On its own passage
it beat neutral on 15 of 16 (0.946 against 1.959), against 10 and 12 for the noise
atlases. So a guitar-built atlas is a much better lookup than a noise-built one,
and still not a better start for a search.
...
infrastructure, while nearest lookup stayed the product behavior — until the
atlases were removed (M7-1 closed, below). Any later model
...
The M7 sections above describe files that are now in git history, and so does
`packs/morgan/warm_start_benchmark_pr12.json`, which stays as M7-2's record and
names the PR12 atlas it was measured on. To have the atlases back exactly as
measured, without staging them:

```bash
git restore --source=55490f9 --worktree -- 'packs/*/response_atlas_*.json'
```

`.gitignore` keeps them from being committed again. The three Morgan topologies
they were built on are frozen in `tests/atlas_topologies.json` and checked against
the templates, so the build commands above still rebuild the same atlases.

On the synthetic chain, building without `--template` works for PR12 and AC20,
...
1.48 against the neutral start's 1.665 on the same targets — closer on only 6 of
12 (paired Wilcoxon p = 0.47), so no gain is shown at all — and at three times
...
answers landed through the guitar; the same gap is 0.06 for a search through the
target's own passage, so the noise misleads it by far more than the scale
difference between the two scores (the search's includes a prior-deviation term
and is its best render of about 300). A DI of a *different song* by the same
player ended 38% closer than no DI (0.92) and closer than neutral on 11 of 12 (p
= 0.007), though still no better on average than the inversion alone through the
right DI (0.871). Matching the probe's loudness to the passage did not help on
average — closer than plain noise on 6 of 12, p = 0.97 — although it moved
individual targets by a lot in both directions, and only integrated loudness was
matched, not peak level into the amp. What a played DI carries that noise lacks
— pitch, sustain, a player's attack — is not separated here. One amp, one target
passage, one player.
...
Wilcoxon p = 0.007, though only p = 0.15 by a sign test); through Hotel California
it was no better (7 of 12, p = 1.0). Its first result did not replicate through
the second passage, and two passages cannot say why — chance, the passage, or
loudness (the synthetic guitar sits 1.4 dB above How Long and 2.8 dB below Hotel
California) — so matches keep the noise probe. It stays in the repository as
`--signal guitar` so this is reproducible. The real DI of the other song, by
contrast, beat noise in every run and both directions — on 11, 12 and 11 of 12
targets (p ≤ 0.002): for one player and two songs on one amp, a DI of another
part was worth asking for. Between the two How Long runs the arms' means moved by
up to 0.15, but single targets by up to 0.36 (own passage), 0.52 (other song) and
0.91 (noise), which is why only within-run pairs are comparisons and why a single
9 of 12 is fragile.
...
149 minutes. The How Long run was made from a pre-squash commit of #57 and its
...
|---|---:|---:|
| the target's own passage — a paired DI | 0.335 / 0.270 | 12 of 12 |
| the same player's Hotel California passage | 0.719 / 0.676 | 12 of 12 |
| the noise probe, as with no DI | 1.132 / 1.138 | — |
| *no search:* neutral settings | 1.465 / 1.238 | 4 of 12 |
| *no search:* the inversion alone, through the target's own passage | 0.674 / 0.649 | 11 of 12 |
| *no search:* the inversion alone, through the Hotel California passage | 0.888 / 0.822 | 10 of 12 |
| *no search:* the inversion alone, through the noise probe | 1.288 / 1.242 | 4 of 12 |
...
p < 0.001 each; SW50R: 38%) — and the target's own passage closer than both on
every target. As on SW50R, a search through the other song ended about where the
inversion alone through the right DI did (0.719 against 0.674, closer on 5 of 12,
p = 0.68), and beat its own starting inversion (10 of 12, 19%, p = 0.009). Even
with no search at all, the inversion through the other song was closer than the
full search through noise on 10 of 12 targets.
...
silence, so what it fits is mostly the note's sustain through the amp. It was
validated on decaying noise bursts, where that is the reverb.
...
fresh process per render, estimate in seconds (confidence):
...
| rack reverb, decay 5 | 9.07 (0.60) | 5.45 (0.91) |
| rack reverb, decay 15 | 6.69 (0.20) | 15.6 (0.62) |
...
| rack reverb, decay 0.5 s | 5.82 (0.46) | 1.83 (0.61) |
| rack reverb, decay 1 s | 4.66 (0.46) | 1.29 (0.64) |
| rack reverb, decay 2 s | 6.15 (0.33) | 2.08 (0.53) |
| rack reverb, decay 4 s | 6.60 (0.20) | 2.30 (0.94) |
| rack reverb, decay 8 s | — | 17.9 (0.46) |

(Morgan's decay is the plugin's 1–60 scale; Tone King's is seconds; mix 50%,
spring off.) It does not. The same dry SW50R reads 1.55 s through one passage
and 6.54 s through the other; Tone King's 0.5 s reverb reads 5.82 s through How
Long; neither passage rises with the decay, and a confidence of 0.8 or 0.9 comes
with readings as wrong as the rest. So a difference between two estimates is
not a difference in reverb — least of all across two different performances,
which is every unpaired comparison.

Second, how often does it reach the score? Each of the search-signal targets and
...
14.3 (the neutral start reads 4.56 s at 0.38, and one target's two renders of
the same settings differed by 2.0 on their own); on Tone King on 2 of 12 this
time (13.8 to 16.4), where the traced pass above found 4. So every SW50R number
in this section carries it too, mostly as a fixed offset per target, since
...
settings after different render histories — moved by 0.100 on average (at most
0.735) for the neutral start and 0.035 (at most 0.375) for the inversions under
...
2.07, where -v1's second run had eight scores from 2.3 to 4.7. Recovery of the
settings did not change: parameter MAE 0.210, 0.233 and 0.273 against -v1's
0.208, 0.238 and 0.267, target by target within 2% and p ≥ 0.57 for each signal
— the term was noise to the search, not information.
...
The baseline passes are the third command with `--workers 1` or `2` and
`--loss-profile unpaired-v1` or `unpaired-v2`, in
`docs/search-signal-toneking-baselines-{v1,v2}-w{1,2}.json`; their `command`
fields record scratchpad paths. The studies took about three minutes each, the
four baseline passes about three each, and the search 110 minutes. The v2 search
started from 147bde6 and its JSON records b62086d, the commit checked out when it
finished; the one between changed only the match and compare defaults, the skill
and tests. `analysis/listening.py` still scores listening comparisons with
`unpaired-v1`, so its objective-versus-listener agreement carries the term too.
...
| The target is unmatchable — layered guitars, mix-bus processing, mastering | **Certain, for commercial masters** | Say so. The atlas (M7-1) was meant to turn this into a specific statement about what the plugin can reach; measured on a played guitar it could not, and it was removed. A preset is not a mix. |
...
plugin the rack reverb then does not repeat itself. Identical settings through
the played DI, rendered back to back on a reused instance, came out 0.435 apart
in `unpaired-v1`, and 0.27 to 0.57 after other settings; a 6 s warm-up (which
plays the DI through the new settings before capture) brought the back-to-back
...
is a tail running into the next render — at most 0.15 through the played DI after
other settings, with the rack reverb off — and the warm-up clears it completely.
...
exposes it where the noise does not. So a match with the tremolo on (any DI), or
the rack reverb on with a played DI, needs `--process-policy fresh` as AC20 does,
at AC20's cost.
...
tail reading cannot tell a spring from a rack reverb. With the neutral render
reading a confident 1.30 s tail through the probe, it stopped all 7 spurious
switch-ons and also dropped 11 of 11 targets that really had a rack reverb;
through the played DI the neutral render's tail was not confident (0.165), so it
changed nothing either way. The rule is unchanged. The atlas benchmark holds its
topology fixed instead ("M7-1 at equal budget"), because what it measures is
starting values.
