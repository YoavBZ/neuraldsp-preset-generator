...
def test_changed_frozen_v2_prediction_is_not_replaced_at_verdict_time(
    ...
    recorded = run("log_blind_verdict.py", "--key", key_path, "--choice", "A",
                   "--listener", "frozen-score-check", "--data-dir", tmp_path / "data")
    assert recorded.returncode == 0, recorded.stdout + recorded.stderr
    assert "objective unscored" in recorded.stderr
    sidecar, = audition_dir.glob("*.objective-verdict.json")
    result = json.loads(sidecar.read_text())
    assert "frozen listening prediction changed: match_v2" in result["scoring_error"]
    assert "objective_scoring" not in result and "agreement_match_v2" not in result
    with Store(str(run_dir / "trials.sqlite3")) as store:
        assert len(store.verdicts(key["match"]["run_id"])) == 1
